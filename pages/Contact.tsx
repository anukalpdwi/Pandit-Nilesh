    ...
    <div className="bg-spiritual">
      ...
      <div className="bg-maroon-900 pt-32 pb-16 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <span className="text-gold-400 font-bold uppercase tracking-widest text-sm">{t.contact.title}</span>
          <h1 className="text-4xl md:text-6xl font-serif font-black mt-4 text-white">{t.contact.subtitle}</h1>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {BOOKING_STEPS.map((step, idx) => (
                <div key={idx} className="text-center relative">
                  <div className="w-20 h-20 bg-gradient-to-br from-saffron-100 to-saffron-50 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg border border-saffron-200">
                    <span className="text-4xl text-maroon-800">{step.icon}</span>
                  </div>
                  <div className="absolute top-10 left-[60%] w-[80%] h-0.5 bg-saffron-200 hidden lg:block last:hidden"></div>
                  <span className="inline-block px-3 py-1 bg-maroon-700 text-white text-xs font-bold rounded-full mb-3">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1 space-y-8">
              <div className="bg-white p-8 rounded-3xl space-y-8 shadow-lg border border-gray-100">
                <h2 className="text-2xl font-serif font-bold text-maroon-900 border-b border-gray-100 pb-4">{t.contact.info}</h2>

                <a href={`tel:${PHONE_NUMBER}`} className="flex items-start space-x-4 group">
                  <div className="p-3 bg-saffron-50 rounded-xl text-maroon-700 shadow-sm group-hover:bg-maroon-700 group-hover:text-white transition-colors">
                    <Phone size={24} />
                  </div>
                  <div>
                    <h4 className="font-bold text-gray-900">{t.contact.callUs}</h4>
                    <p className="text-lg text-maroon-700 font-bold">+{PHONE_NUMBER}</p>
                ...
                <div className="flex items-start space-x-4">
                  <div className="p-3 bg-saffron-50 rounded-xl text-maroon-700 shadow-sm"><MapPin size={24} /></div>
                  <div>
                ...
                <div className="flex items-start space-x-4">
                  <div className="p-3 bg-saffron-50 rounded-xl text-maroon-700 shadow-sm"><Clock size={24} /></div>
                  <div>
                    ...
                    <p className="text-xs text-saffron-600 font-medium mt-1">{lang === 'en' ? 'Puja timings may vary' : 'पूजा का समय भिन्न हो सकता है'}</p>
                  </div>
                </div>
              </div>

              {/* WhatsApp Card */}
              <div className="bg-gradient-to-br from-green-700 to-green-800 p-8 rounded-3xl text-white relative overflow-hidden group shadow-xl">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-125 transition-transform">
                  <MessageSquare size={100} />
                </div>
                <h3 className="text-xl font-serif font-bold mb-4">{t.contact.directChat}</h3>
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl shadow-2xl p-8 md:p-12 border border-gray-100">
              <h2 className="text-2xl font-serif font-bold text-maroon-900 mb-8 flex items-center space-x-3">
              ...
              <form onSubmit={handleWhatsAppSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-saffron-500 focus:bg-white transition-all"
                ...
                <div className="md:col-span-2 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-maroon-800 to-saffron-600 text-white py-5 rounded-xl font-bold text-lg hover:from-maroon-900 hover:to-saffron-700 shadow-xl transition-all flex items-center justify-center space-x-3"
                    >
                      <Send size={24} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20">
          <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl">
            <div className="text-center mb-12">
              <span className="text-saffron-600 font-bold uppercase tracking-widest flex items-center justify-center space-x-2 text-sm">
                <IndianRupee size={20} />
                <span>{t.pricing.title}</span>
              </span>
              <h2 className="text-3xl md:text-4xl font-serif font-black mt-4 text-maroon-900">{t.pricing.subtitle}</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {PRICING_DATA.map((item, idx) => (
                <div key={idx} className="bg-gradient-to-br from-saffron-50 to-white p-6 rounded-2xl border border-saffron-100 hover:shadow-lg transition-all group">
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-bold text-gray-900 group-hover:text-maroon-800 transition-colors">{item.puja}</h3>
                    <CheckCircle className="text-green-600 flex-shrink-0" size={20} />
                  </div>
                  <div className="text-2xl font-black text-saffron-600 mb-2">{item.price}</div>
            ...
            <div className="mt-8 p-6 bg-saffron-50 rounded-2xl border border-saffron-100">
